...
function listings( $aType, $input, $args, $parser ) {
	...
	if (isset( $args['name'] )) $name = $args['name'];
		else $name = wfMsgForContent( 'listingsUnknown' );
	if (isset( $args['alt'] )) $alt = $parser->internalParse($args['alt']); else $alt = '';
	if (isset( $args['address'] )) $address = $parser->internalParse($args['address']); else $address = '';
	if (isset( $args['directions'] )) $directions = $parser->internalParse($args['directions']); else $directions = '';
	if (isset( $args['phone'] )) $phone = $args['phone']; else $phone = '';
	if (isset( $args['tollfree'] )) $tollfree = $args['tollfree']; else $tollfree = '';
	if (isset( $args['email'] )) $email = $args['email']; else $email = '';
	if (isset( $args['fax'] )) $fax = $args['fax']; else $fax = '';
	if (isset( $args['url'] )) $url = $args['url']; else $url = '';
	if (isset( $args['hours'] )) $hours = $args['hours']; else $hours = '';
	if (isset( $args['price'] )) $price = $args['price']; else $price = '';
	if (isset( $args['checkin'] )) $checkin = $args['checkin']; else $checkin = '';
	if (isset( $args['checkout'] )) $checkout = $args['checkout']; else $checkout = '';
	if (isset( $args['lat'] ) and is_numeric($args['lat']) ) $lat = $args['lat']; else $lat = 361;
	if (isset( $args['long'] ) and is_numeric($args['long']) ) $long = $args['long']; else $long = 361;
	if (isset( $args['tags'] )) $tags = $args['tags']; else $tags = 0;
	...
	if ($url != '') $out = '<a href="' . $url . '" class="external text" rel="nofollow" title="' . $name . '">' . $out . '</a>';
	if ($alt != '') $out .= ' (<em>' . $alt . '</em>)';
	if (($address != '') or ($directions != '') or ($position != '')) {
		$out .= ', ' . $address;
		if (($directions != '') or ($position != '')) {
			$out .= ' (<em>' . $directions;
			if (($directions != '') and ($position != '')) $out .= ', ';
	...
	if ($phoneSymbol != '')
		...
		else $phoneSymbol = wfMsgForContent( 'listingsPhone' ) ;
	...
	if ($faxSymbol != '')
		...
		else $faxSymbol = wfMsgForContent( 'listingsFax' ) ;
	...
	if ($emailSymbol != '')
		...
		else $emailSymbol = wfMsgForContent( 'listingsEmail' ) ;
	...
	if ($tollfreeSymbol != '')
		...
		else $tollfreeSymbol = wfMsgForContent( 'listingsTollfree' ) ;

	if (($phone != '') or ($tollfree != '')) {
		$out .= ', ' . $phoneSymbol . ' ' . $phone;
		if ($tollfree != '') $out .= ' (' .$tollfreeSymbol . ': ' . $tollfree . ')';
	}
	if ($fax != '') $out .= ', ' . $faxSymbol . ': ' . $fax;
	if ($email != '') $out .= ', ' . $emailSymbol . ': ' . '<a class="email" href="mailto:' . $email . '">' . $email . '</a>';
	$out .= '. ';
	...
	if (($checkin != '') or ($checkout != '')) {
		...
		if ($checkout != '') $out .= wfMsgForContent( 'listingsCheckout' ) . ': ' . $checkout;
		$out .= '. ';
	}
	if ($price != '') $out .= $price . '. ';
